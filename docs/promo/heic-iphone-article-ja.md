...
<!-- 📷 スクショ挿入：Windowsで .heic をダブルクリックして「開けない／対応するアプリがない」と出ている画面 -->
...
<!-- 📷 スクショ挿入：iPhoneの「設定 → カメラ → フォーマット」画面（「互換性優先」を選んでいるところ） -->
...
<!-- 📷 スクショ挿入：Filewispの変換画面（ファイルをドロップ→プレビュー→ダウンロードの様子。1枚でも可、3ステップ並べると親切） -->
