...
**対応:** AVIF・PNG透過・Parquetガイドのタイトル、説明、本文見出しを実クエリへ合わせた。英語ParquetガイドへBigQueryのロード／外部テーブルの使い分けと公式資料を追加。JPG→PNGとCSV→Parquetは英語タイトルに `Online` を加え、背景透過・Parquet Viewer・BigQueryガイドへの内部リンクを強化した。

**判断:** 表示数がまだ少ないため、上位候補ページのURLや構成を大きく変えず、検索意図への即答と内部リンクだけを調整。次回レポートでCTRと8〜20位ページの変化を比較する。
...
- [x] 検索パフォーマンスで表示回数・クリックが発生し始めたか確認 → 2026/07/22実行、直近期間は表示21・クリック0・平均順位41.3（`docs/seo-reports/gsc-2026-07-19.md`）
