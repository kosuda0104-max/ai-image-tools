...
import { csvToParquetContent } from "@/src/data/tools/csv-to-parquet";
...
export default function CsvToParquetTool({ locale }: Props) {
  const { page, ui } = csvToParquetContent[locale];
  const [file, setFile] = useState<File | null>(null);
  const [parsed, setParsed] = useState<ParsedCsv | null>(null);
  const [status, setStatus] = useState("");
  const [isProcessing, setIsProcessing] = useState(false);
  const [parquetBlob, setParquetBlob] = useState<Blob | null>(null);
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const columnData = columns.map((name, colIdx) => ({
        name,
        data: data.map((row) => (row[colIdx] !== undefined && row[colIdx] !== "" ? row[colIdx] : null)),
  ...
  const handleDownload = () => {
    if (!parquetBlob) return;
    triggerBlobDownload(parquetBlob, `${baseName}.parquet`);
  };
