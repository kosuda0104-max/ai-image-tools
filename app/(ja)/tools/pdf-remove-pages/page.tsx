...
import { pdfRemovePagesContent } from "@/src/data/tools/pdf-remove-pages";

const locale = "ja";
const t = pdfRemovePagesContent[locale];
...
export default function Page() {
  ...
  const handleRemovePages = async () => {
    ...
    try {
      ...
      const pagesToRemove = parsePageNumbers(pageText, maxPage);

      if (pagesToRemove.length >= maxPage) {
        setStatus(t.ui.resizeError);
        setIsProcessing(false);
        return;
      }

      const zeroBasedIndexes = pagesToRemove
        .map((page) => page - 1)
        .sort((a, b) => b - a);

      for (const index of zeroBasedIndexes) {
      ...
      link.download = `${baseName}-removed-pages.pdf`;
      link.click();
